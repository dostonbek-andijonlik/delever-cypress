class TelegramPost{
    elements = {
        ...
        addPhoto: () => cy.get('#full-width-tabpanel-0 > .MuiBox-root > .MuiTypography-root > .my-4 > .Gallery > .add-block'),
        addVideo: () =>cy.get('.MuiTabs-flexContainer').eq(1).click().get('#full-width-tabpanel-1 > .MuiBox-root > .MuiTypography-root > .my-4 > .Gallery > .add-block'),
        addGIF: () => cy.get('.Mui-selected > .MuiTab-wrapper > .px-1').click().get('#full-width-tabpanel-2 > .MuiBox-root > .MuiTypography-root > .my-4 > .Gallery > .add-block'),
    ...
    searchPost(
        ...
        this.elements.firstTableItem().should('contain', postName)
    ...
    addPost(        
        ...
        if(photoPath!=undefined){            
            this.elements.addPhoto().find('input').selectFile(photoPath, {force:true})
        }
        
        if(videoPath!=undefined){
            this.elements.addVideo().find('input').selectFile(videoPath, {force:true})
        }
